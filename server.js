...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', message => {
        try {
            ...
            // ⚡ BISTURÍ MICELATOR — browser → KP
                        if (data.type === 'MICELATOR_TASK') {
                            if (kpSocket && kpSocket.readyState === WebSocket.OPEN) {
                                kpSocket.send(msgString);
                                console.log(`[Micelator] ⚡ → KP: "${data.task}"`);
                            } else {
                                ws.send(JSON.stringify({ type: 'MICELATOR_ACK', ok: false, error: 'KP offline' }));
                            }
                            return;
                        }
